...
(string/split-lines
 (:out (sh "ls" "-lh" "my_figure.png" "my_figure.svg")))
...
^kind/vega
(-> (map (fn [x] {:x x :y (dtype-func/sin (- x 0))})
         x-range)
    (hanami-plot hanami-templates/line-chart))
...
(def with-manual-colors
  (-> []
      (into
       (map
        (fn [x]
          {:x x :y (dtype-func/sin (- x 0)) :color "blue"})
        x-range))
      (into
       (map
        (fn [x]
          {:x x :y (dtype-func/sin (- x 1)) :color "green"})
        x-range))
      (into (map (fn [x]
                   {:x     x
                    :y     (dtype-func/sin (- x 2))
                    :color "hsl(0, 0%, 75%)"})
                 x-range))
      (into (map (fn [x]
                   {:x     x
                    :y     (dtype-func/sin (- x 3))
                    :color "#FFDD44"})
                 x-range))
      (into (map (fn [x]
                   {:x     x
                    :y     (dtype-func/sin (- x 4))
                    :color "rgb(256, 51, 77)"})
                 x-range))
      (into (map (fn [x]
                   {:x     x
                    :y     (dtype-func/sin (- x 5))
                    :color "chartreus"})
                 x-range))))
  ...
  (->> []
       ...
       (into (map (fn [x]
                    {:label  1
                     :x      x
                     :y      (+ x 1)
                     :stroke (:dashed linestyle)})
                  x-range))
       (into (map (fn [x]
                    {:label  2
                     :x      x
                     :y      (+ x 2)
                     :stroke (:dasheddot linestyle)})
                  x-range))
       (into (map (fn [x]
                    {:label  3
                     :x      x
